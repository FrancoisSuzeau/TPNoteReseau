...
void InGame(SDL_Surface *screen)
{
    ...
    while(continuer)
    {
            ...
            case SDL_KEYDOWN:
                switch(event.key.keysym.sym)
                {
                    ...
                    case SDLK_RIGHT:
                        ...
                        break;
                    case SDLK_SPACE:
                        select_pawn = selectPawn(my_manBoard, &position_selector, &updatePosition);
                        break;
                    default:
                        break;
                }
        ...
        if((updatePosition.x != 0) && (updatePosition.y != 0))
        {
            my_manBoard->boardSDL[updatePosition.y][updatePosition.x] = EMPTY;
            updatePosition.x = 0;
            updatePosition.y = 0;
        }
        updateMapPawn(screen, black_pawn, white_pawn, my_manBoard);
    ...
    if(manBord->selectIs == EMPTY) // the selector is empty
    {
        ...
        else if(manBord->board_blackCase[posi->y][posi->x] != 0) // the selector is on brown case
        {
            if(manBord->boardSDL[posi->y][posi->x] == BLACK) //the case is not empty and with a black paw
            {
                ...
                if((validateSelect(manBord, posi) == TRUE) || validateTaken(manBord, posi, newPosition) == TRUE)
                {
